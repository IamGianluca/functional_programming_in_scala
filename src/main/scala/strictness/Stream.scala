...
sealed trait Stream[+A] {
  ...
  /** Generalise tails. This function is like a foldRight that returns a sstream of the intermediate results
    * Source: https://github.com/fpinscala/fpinscala/blob/master/answers/src/main/scala/fpinscala/laziness/Stream.scala
    *
    * The function can't be implemented using `unfold`, since `unfold` generates elements of the `Stream` from left to
    *   right. It can be implemented using `foldRight` though. The implementation is just a `foldRight` that keeps the
    *   accumulated value and the stream of intermediate results, which we `cons` onto during each iteration. When
    *   writing folds, it's common to have more state in the fold than is needed to compute the result. Here, we simply
